...
from helpermethods import create_employee_worker, update_employee
...
def addemployee(request):
    ...
    """
    form_list = form_generator(3)
    form_list[0] = AddEmployeeForm(request.POST or None)
    form_list[1] = AddressForm(request.POST or None)
    form_list[2] = EmployeeContactForm(request.POST or None)

    validation = validation_helper(form_list)
    if validation:
        address = create_address_helper(form_list[1])
        contact = create_employee_contact_helper(form_list[2])
        employee = create_employee_worker(form_list[0], address, contact)
        return HttpResponseRedirect(reverse('Employee:index'))

    return render(request, 'employee/addemployee.html', dict_generator(form_list))


def editemployee(request, pk):
    """
    View for editing an Employee object.
    @param request: request.
    @param pk: Employee PK.
    @return: HTTP Request or Form.
    """
    form_list = form_generator(3)
    ...
    form_list[0] = EmployeeForm(request.POST or None, instance=employee)
    form_list[1] = AddressForm(request.POST or None, instance=address)
    form_list[2] = EmployeeContactForm(request.POST or None, instance=contact)

    validation = validation_helper(form_list)
    if validation:
        update_address_helper(form_list[1], address)
        contact_updated = update_contact_employee_helper(form_list[2], contact)
        update_employee(form_list[0], employee, address_updated, contact_updated)
        return HttpResponseRedirect(reverse('Employee:index'))
    form_dict = dict_generator(form_list)
